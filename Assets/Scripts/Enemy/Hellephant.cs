...
public class Hellephant : EnemyBase, IMoveable
{
    ...
    protected override void UpdateTrace()
    {
        ...
        if (Target != null && Vector3.Distance(Target.position, transform.position) <= data.AttackDistance)
        {
    ...
    protected override void UpdateAttack()
    {
        if (Time.time > _lastAttack + data.AttackInterval)
        {
            ...
            target.OnDamage(data.Damage, Target.position, transform.position);
